...
namespace acme {
  ...
  ACMEClient::~ACMEClient() { shutdown(); }
  ...
  bool ACMEClient::initialize() {
    ...
    std::string server_url =
        config.get("server_url", "http://localhost:8080").asString();
    std::string kdc_principal =
        config.get("kdc_principal", "acme/client@REALM").asString();
    std::string kdc_keytab =
        config.get("kdc_keytab", "/etc/acmecli/client.keytab").asString();
  ...
  Json::Value ACMEClient::getAccount(const std::string &account_id) {
    Json::Value response =
        sendRequest("/acme/account/" + account_id, "GET", Json::Value());
    return response;
  }
  ...
  Json::Value ACMEClient::getOrder(const std::string &order_id) {
    Json::Value response =
        sendRequest("/acme/order/" + order_id, "GET", Json::Value());
    return response;
  }

  Json::Value ACMEClient::finalizeOrder(const std::string &order_id,
                                        const Json::Value &request) {
    Json::Value response =
        sendRequest("/acme/order/" + order_id + "/finalize", "POST", request);
    return response;
  }

  Json::Value ACMEClient::getCertificate(const std::string &cert_id) {
    Json::Value response =
        sendRequest("/acme/cert/" + cert_id, "GET", Json::Value());
    return response;
  }
  ...
  Json::Value ACMEClient::sendRequest(const std::string &path,
  ...
  bool ACMEClient::createCSR(const std::string &common_name,
                             std::string &csr_pem) {
  ...
  X509 *ACMEClient::generateX509(const std::string &common_name, EVP_PKEY *key,
                                 bool is_ca) {
    X509 *cert = X509_new();
    if (!cert) {
      return nullptr;
  ...
  void ACMEClient::addExtensions(X509 *cert, const std::string &common_name,
                                 bool is_ca) {
    // Basic constraints
    X509_EXTENSION *ext =
  ...
  OM_uint32 ACMEClient::acquireCredentials(const std::string &principal,
                                           gss_cred_id_t *cred) {
    ...
    maj_stat =
        gss_acquire_cred(&min_stat, name, GSS_C_INDEFINITE, GSS_C_NO_OID_SET,
                         GSS_C_INITIATE, cred, nullptr, nullptr);
  ...
  OM_uint32 ACMEClient::initSecurityContext(const std::string &target_principal,
    ...
    maj_stat = gss_init_sec_context(
        &min_stat, client_cred, context_ptr, GSS_C_NO_NAME, GSS_C_NO_OID,
