...
void printUsage(const char* program_name) {
    console::e("Usage: <command> [options]\n\n"
              "Commands:\n"
...
void registerAccount(ACMEClient& client) {
    ...
    if (response.isMember("status") && response["status"].asString() == "valid") {
...
void createOrder(ACMEClient& client, const std::string& common_name) {
    ...
    if (response.isMember("status") && response["status"].asString() == "pending") {
...
void finalizeOrder(ACMEClient& client, const std::string& order_id, const std::string& csr_file) {
...
int main(int argc, char* argv[]) {
    ...
    int opt;
    int option_index = 0;
    while ((opt = getopt_long(argc, argv, "s:n:", long_options, &option_index)) != -1) {
        switch (opt) {
            case 's':
    ...
    ACMEClient client(config_file.empty() ? "/etc/acmecli/acme_client.conf" : config_file);
    ...
    if (command == "init") {
        ...
        if (account_id.empty() || csr_file.empty()) {
            console::e("Error: --account-id and --csr options are required for finalize-order command");
            return 1;
        }
        finalizeOrder(client, account_id, csr_file);
    } else if (command == "get-certificate") {
        if (cert_id.empty()) {
            console::e("Error: --cert-id option is required for get-certificate command");
            return 1;
        }
