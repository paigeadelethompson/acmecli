...
KerberosAuth::KerberosAuth(const std::string& principal, const std::string& keytab_path)
...
bool KerberosAuth::initialize() {
    ...
    console::e("Kerberos authentication initialized for principal: {}", principal_);
    return true;
}

bool KerberosAuth::authenticateClient(const std::string& token, std::string& principal) {
...
OM_uint32 KerberosAuth::acquireCredentials() {
    ...
    maj_stat = gss_acquire_cred(&min_stat, name, GSS_C_INDEFINITE, GSS_C_NO_OID_SET, GSS_C_ACCEPT, &server_cred_, nullptr, nullptr);
...
OM_uint32 KerberosAuth::verifyToken(gss_ctx_id_t context, const std::string& token, std::string& principal) {
    OM_uint32 maj_stat, min_stat;
    ...
    maj_stat = gss_accept_sec_context(&min_stat, &context, server_cred_, &input_token, GSS_C_NO_CHANNEL_BINDINGS, &client_name, &mech_type, &output_token, &ret_flags, &time_rec, &delegated_cred);
...
OM_uint32 KerberosAuth::wrapToken(gss_ctx_id_t context, const std::string& token, std::string& wrapped_token) {
    OM_uint32 maj_stat, min_stat;
    gss_buffer_desc input_token;
    gss_buffer_desc output_token;
...
OM_uint32 KerberosAuth::unwrapToken(gss_ctx_id_t context, const std::string& wrapped_token, std::string& token) {
    OM_uint32 maj_stat, min_stat;
    gss_buffer_desc input_token;
    gss_buffer_desc output_token;
