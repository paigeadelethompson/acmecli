...
CertificateManager::CertificateManager(const std::string& ca_key_path,
    ...
    : ca_key_path_(ca_key_path),
      ...
      server_cert_path_(server_cert_path) {
}
...
bool CertificateManager::createServerCertificate() {
    ...
    X509* server_cert = generateX509("acme-server.example.com", server_key, false);
    if (!server_cert) {
...
bool CertificateManager::generateCertificate(const std::string& common_name, const std::string& csr_pem) {
    // Parse CSR
...
X509* CertificateManager::generateX509(const std::string& common_name, EVP_PKEY* key, bool is_ca) {
    X509* cert = X509_new();
    if (!cert) {
        return nullptr;
    }
...
void CertificateManager::addExtensions(X509* cert, const std::string& common_name, bool is_ca) {
    // Basic constraints
    ...
    // Key usage
    ext = X509V3_EXT_conf_nid(nullptr, nullptr, NID_key_usage, "digitalSignature,keyCertSign,cRLSign");
    if (ext) {
    ...
    ext = X509V3_EXT_conf_nid(nullptr, nullptr, NID_authority_key_identifier, "keyid,issuer");
    if (ext) {
